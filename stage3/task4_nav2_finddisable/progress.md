...
## 数据解读小结（T5.3① 预测-实测对照·阶段性，②③ 课后补测后补全）

| 注入 | 预注册假设 | 实测 | 启封结论 |
|---|---|---|---|
| ① 动态障碍 | 预计重规划 | 恢复用尽 → Goal failed，0/1 自恢复（findings #30） | **偏差**：不是绕行/重规划，是恢复无效失败 |
| ④ 障碍内目标 | 预计带原因 FAILED | navigate_to 服务侧＝**是**（0.0s 带原因"目标位于障碍物内"）；Nav2 原生对照＝无原因 Goal failed（findings #32） | 假设针对服务接口成立 |
| ② 遮挡 30% | 预计安全降级 | 实验无效（注入未进入系统，findings #31） | **不启封**，课后补测 |
| ③ 先验偏置 1m | 预计重新收敛 | 实验无效（注入未送达，findings #35） | **不启封**，课后补测 |
| 实车绑架 | 预计不能 | 课堂公共基线抄录待做 | — |

- **孪生自恢复数**：有效实验仅 ① → **0/1**；②③ 补测后填 __/3。
- ③ 附基线（供补测对照）：AMCL 漂移 0.27m→0.34m，到达判定基于估计位姿（真值差 0.34m 仍判 succeeded）。
- SR@d_th（T5.3② 选做）与回归 run_action1（T5.3③ 必做）待课后。
...
- 阶段 3：① ✓ 闭环（恢复用尽→Goal failed，findings #30）；④ 已执行已回写（Goal failed 55.3s，无录像，findings #32）；② 无效注入已回写待补测（findings #31）；③ 主试无效已归档（findings #35，工具已修，课后补测）。**障碍表已更正为真值 6 障碍（findings #33）**
- ②③④ injectx 文件已归档入仓库（config/nav2_params_inj2.yaml、logs/traj_G42_inject{2_mask,3_amcl,4_invailable}.csv、videos/G42_遮挡30%_注入未生效.webm、G42_定位扰动_注入未生效.webm，sha256 与 ~/sim_ops 一致）
- 收尾：轨迹元数据索引（logs/轨迹元数据.md，10 份 CSV 统计+sha256）；能力边界声明三条候选稿（能力边界声明.md，待互问定稿）；T5.3① 预测-实测对照阶段性小结（上节）；预注册单冻结 SHA/生成时间已填
...
5. 实车公共基线抄录（课堂）；能力边界声明候选稿已出（待组员互问定稿签字）；六行记录表完稿（①②③④ 已写，剩实车两行抄录）
6. 数据解读：预测-实测对照阶段性完成（见「数据解读小结」）；剩 SR@d_th 选做、回归 run_action1（改过膨胀→必做）、②③ 补测后补全对照

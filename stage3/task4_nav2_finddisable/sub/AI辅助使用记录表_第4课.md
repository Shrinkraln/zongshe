...
| 7 | 能力边界声明.md 三条候选（Claude 依 ①④ 行与 ③ 基线数据起草）+ logs/轨迹元数据.md | 声明为 AI 归纳，未经组员按 T5.1 判据互问验证；元数据统计为脚本复算（与记录表行文已交叉核对） | 组员互问（他组能否复现）后定稿签字；元数据 sha256 可 `sha256sum` 复核 | "AI 起草边界声明候选与轨迹元数据／组员互问定稿后采纳" |
